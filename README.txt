### Description of reproducible codes for "Bayesian high-dimensional covariate selection in non-linear mixed-effects models using the SAEM algorithm" Naveau et al.

This folder contains the R codes needed to reproduce the results of the article.

## Scripts for Sections 3 and 4:

App_SAEMVS_ex.Rmd: This file contains an application of SAEMVS on a detailed example. The HTML version of this Markdown is included in the Supplementary Web Material. This file needs the functions that are listed in Functions_SAEMVS.R

Function_SAEMVS.R: This file contains all the functions needed to run the SAEMVS procedure on the detailed example, that is:
...
## Scripts for Section 5: Simulation study and comparison with MCMC

Functions_SAEMVS_simu.R: This file contains all the functions needed to run the SAEMVS procedure on the simulation study model, that is all the functions present in Functions_SAEMVS but adapted according to Appendix A.3.
...
nimble_fit_functions.R: This file contains functions required for the efficient C++ implementation of a random walk adaptive MCMC used in Sub-section 5.4 through the Nimble software.

Fig3.R: Script to reproduce Figure 3

Test_Simu_SAEMVS.R: This file allows to combine the functions of the file "Functions_SAEMVS_simu.R" to launch the SAEMVS method on the model as presented in the simulation study Section 5 on one simulated data-set.
...
Fig5.R: Script to reproduce Figure 5. This file needs the functions that are listed in Functions_SAEMVS_real_data.R

Functions_SAEMVS_real_data.R: This file contains all the functions needed to run the SAEMVS procedure on the real dataset presented in Section 6, that is all the functions present in Functions_SAEMVS but adapted according to Section 6.

Fig6.R: Script to reproduce Figure 6


## Data and Rdata

13INRmon_NUE_LN.csv: dataframe containing the observed data

carte_Axiom-TABW420k_WGAv1.csv: dataframe containing the position on the genome of each SNP, data_chromosome$V1: chromosome, data_chromosome$V2: position on the chromosome, data_chromosome$name: name of the SNP

GenesMajeursFloraison.csv: dataframe containing the position on the genome of each flowering gene

genotypeFilterPlink_100_5_0.8.csv: dataframe containing the value of each covariate (that is the SNPs) for each variety

marker_HD_INRmon13LN.csv: list of heading QTLs names

resPCOAdf.Rds: the first 5 principal components of a PCA performed on the available SNPs in the model
