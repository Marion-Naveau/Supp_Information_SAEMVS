...
#Function that calculates the conditional expectation of delta knowing Theta
...
SAEM_MAP <- function(niter,nburnin,niterMH_phi,niterMH_psi,Y,t,id,V_tilde,param_init,hyperparam,s) {
  ...
  for (k in 1:niter) {
    ...
    sigma2[k+1]=max(tau[1]*sigma2[k],(nu_sigma*lb_sigma+s1[k+1])/(n*J+nu_sigma+2))
...
SAEM_EMV <- function(niter,nburnin,niterMH_phi,niterMH_psi,Y,t,id,V_tilde,param_init,hyperparam,I,s){
  #Input notations identical to SAEM_MAP except:
  #hyperparam = list(Omega,tau) where Omega=vector containing the diagonal elements of Omega, and tau=annealing parameter
  ...
  s2 = rep(0,niter+1)                    #stat associated with the sum of phi^2
  s3 = matrix(0,nrow=n,ncol=niter+1)     #stats associated with phi_i
  s4 = matrix(0,nrow=2,ncol=niter+1)     #stat associated with psi
  mco=matrix(0,n,J)                      #will be useful for the s1 update
  ...
  psi=matrix(NA,nrow=2,ncol=niter+1)
  phi[,1] <- V_tilde%*%beta_tilde[,1]
  psi[,1] <- eta[,1]
  for (k in 1:niter) {
    ## S-step: Metropolis Hastings within Gibbs
    phi_tilde=matrix(NA,nrow=n,ncol=niterMH_phi+1)
    psi_tilde=matrix(NA,nrow=2,ncol=niterMH_psi+1)
    phi_tilde[,1]=phi[,k]
    psi_tilde[,1]=psi[,k]
    Vbetak=V_tilde%*%beta_tilde[,k]
    ...
    for (m in 1:2){
      for (r in 1:niterMH_psi){
        psi_mc=rnorm(1,mean=eta[m,k],sd=sqrt(Omega[m]))
        phi_k=rep(phi[,k+1],each=J)
        if (m==1){
          psi_c=c(psi_mc,psi[2,k])
          psi_r=c(psi_tilde[1,r],psi[2,k])
    ...
    s2[k+1]=s2[k]+gamma[k]*(sum(phi[,k+1]^2)-s2[k])
    s3[,k+1]=s3[,k]+gamma[k]*(phi[,k+1]-s3[,k])
    s4[,k+1]=s4[,k]+gamma[k]*(psi[,k+1]-s4[,k])

    ## M-step: update of (beta_tilde,Gamma2,sigma2,eta)
    V_hat=V_tilde[,I]
    tV_hat=t(V_hat)
  ...
  return(list(beta_tildeEMV=beta_tilde[,niter+1],Gamma2EMV=Gamma2[niter+1],sigma2EMV=sigma2[niter+1],psiEMV=eta[,niter+1]))
}


Model_selection <- function(Delta,niter,nburnin,niterMH_phi,niterMH_psi,Y,t,id,V_tilde,param_init,hyperparam,s){
  #Input notations identical to SAEM_MAP except:
  ...
  threshold_support<-foreach(m = 1:M, .packages = c("nlme","mvnfast","doParallel","ggplot2","cowplot","glmnet","microbenchmark")) %dopar% {
    ...
    res = SAEM_MAP(niter,nburnin,niterMH_phi,niterMH_psi,Y,t,id,V_tilde,param_init,hyperparam,s)
    ...
    result= list(threshold=threshold,support=support,beta_tildehat=beta_tildehat,Gamma2hat=Gamma2hat,alphahat=alphahat,sigma2hat=sigma2hat,etahat=etahat)
    result
  }
  stopCluster(cl)
  ...
  etahat=matrix(NA,nrow=2,ncol=M)          #eta MAP estimate for each value of nu0
